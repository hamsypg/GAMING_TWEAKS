...
:: ########  RESTORE POINT  ########
:enrestorepoint
Title RESTORE POINT *IMPORTANT* & color c
cls
echo.
echo	 If you wanna Skip Creating Restore Point Type "SKIP"
echo	 If you wanna Create Restore Point Type "CREATE"
echo.
set /p enrestorepoint="Write (SKIP or CREATE): "

if "%enrestorepoint%"=="" goto enrestorepoint 
if /i "%enrestorepoint%"=="SKIP" goto skiprestorepoint 
if "%enrestorepoint%"=="CREATE" goto createrestorepoint

if /i "%enrestorepoint%" neq "SKIP" if /i "%enrestorepoint%" neq "CREATE" (
cls
echo.
echo	 Error. Please Write (SKIP or Create).
goto enrestorepoint
)
...
echo.
wmic.exe /Namespace:\\root\default Path SystemRestore Call CreateRestorePoint "Befor GAMING TWEAKS BY HAMSYPG", 100, 7
echo.
echo	 Creating Restore Point Complete.
echo.
timeout /t 3
goto ln
...
cls
echo.
echo	 Creating Restore Point SKIPED...
echo.
timeout /t 3
goto ln
...
if not defined ln goto lnerror
if /i %ln%==EN goto EN goto lnerror
if /i %ln%==FR goto FR goto lnerror

:lnerror
cls
title ERROR & color c
echo.
echo	 Error Choice Correct Option (Please Type EN or FR)
echo.
timeout /t 3
goto ln
...
cls
	echo.
	echo	 La langue francaise n'est pas encore prete,
	echo	    elle sera disponible dans la prochaine mise a jour
	echo.
	echo	 Mais nous vous avons fourni une copie de sauvegarde
	echo	      qui fonctionne pour les systemes francais
	echo.
pause
cd Resources\FR
start GAMING_TWEAKS_by_HAMSYPG_beta_FR_v0.3.bat
exit
