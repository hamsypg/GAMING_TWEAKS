...
Del /S /F /Q C:\WINDOWS\Prefetch
timeout /t 3
echo Supprimez les fichiers temporaires. FAIT
cls
...
md c:\windows\temp
del /s /f /q C:\WINDOWS\Prefetch
echo oui | del %appdata%\Microsoft\Windows\Recent
echo oui | del %localappdata%\temp
timeout /t 3
echo Supprimez les fichiers temporaires. FAIT
cls
...
timeout /t 3
echo.
echo Lorsque vous avez termine, fermez la fenêtre pour terminer le processus
echo.
timeout /t 5
dfrgui|defrag /C /D
echo.
...
reg add HKCU\Software\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications /v GlobalUserDisabled /t reg_dword /d "1" /f
reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Search /t reg_sz /d "0" /f
echo Empêchez les applications inutiles de demarrer automatiquement. FAIT
timeout /t 3
cls
